...
The legacy `{from}`, `{subject}`, `{to}`, `{mailbox}`, and `{filepath}` placeholders were removed for shell-injection safety. Configs that still reference them are rejected at load time with a migration hint — replace each with its `$AIMX_*` equivalent inside double quotes.
