...
- **Inbound recipients must be yours.** `RCPT TO` is only accepted for addresses on `config.domain` that resolve to a configured mailbox (or the catchall, if enabled). Mail addressed to any other domain is refused at RCPT TO with a 5xx and never stored.
...
- **No bounce / DSN generation**, because delivery failures on aimx's inbound side (unknown recipient) are reported synchronously in the SMTP dialogue as a 5xx reply. Standard senders handle that correctly. Generating asynchronous DSNs is how backscatter happens; declining to do it is the safer choice.
