...
3. **Set up STARTTLS certificate.** Generates a self-signed certificate at `/etc/ssl/aimx/`. Skipped on re-entry when the cert is already present.
...
When aimx detects an existing configuration (`aimx serve` running, STARTTLS cert present, DKIM key present), the wizard's checklist marks **STARTTLS certificate**, **trust policy**, and **install AIMX** as ☒ (skipped) so you can see at a glance that nothing was rewritten. Steps 1, 2, and 6 still run: port 25 preflight, DNS verification, and the agents-setup drop-through. Re-entry is the natural "I want to wire another agent" checkpoint; the TUI's `(AIMX MCP wired)` state is self-documenting so you will not double-wire anything by accident.
