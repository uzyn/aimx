...
- `status.rs` — `aimx status`: gathers mailbox counts, unread counts, DKIM key presence, SMTP service state (systemd/OpenRC), and recent activity.
...
- `trust.rs` — per-mailbox trust evaluation. `TrustedValue::{None, True, False}` serializes to lowercase strings for the frontmatter `trusted` field (FR-37b); `evaluate_trust()` applies the mailbox's `trust = "none"|"verified"` policy plus `trusted_senders` glob list to the email's DKIM result.
