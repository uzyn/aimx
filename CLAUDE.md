...
- `hook.rs`: hook manager. Event dispatch for `on_receive` (fired by `ingest.rs`) and `after_send` (fired by `send_handler.rs`). Owns the trust gate (`should_fire_on_receive`), the effective-name resolver (`effective_hook_name` / `derive_hook_name`), and the structured `info` log line emitted per fire. Hooks carry an optional `name`; when omitted, the effective name is derived deterministically from `sha256(event + cmd + dangerously_support_untrusted)` and is globally unique across mailboxes (config load rejects collisions).
