...
### 6.6 Hook Manager
- FR-29: Read hook rules from `config.toml` per mailbox — `[[mailboxes.<name>.hooks]]` arrays-of-tables, one entry per hook. Required fields: `event` (`on_receive` | `after_send`), `cmd`. Optional: `name` (matches `^[a-zA-Z0-9_][a-zA-Z0-9_.-]{0,127}$`); when omitted, AIMX derives a stable 12-hex-char name from `sha256(event + cmd + dangerously_support_untrusted)`. Effective names are globally unique across mailboxes (explicit and derived alike). Unknown fields are rejected at config load via `#[serde(deny_unknown_fields)]`. Legacy `[[mailboxes.<name>.on_receive]]` schema refuses to load with a migration error.
- FR-30: Support `cmd` execution: the daemon spawns `sh -c <cmd>` with aimx-controlled template variables `{id}` and `{date}` substituted into the command string. All user-controlled fields are exposed as `AIMX_`-prefixed environment variables to avoid shell-injection via header values. `on_receive` env vars: `AIMX_FROM`, `AIMX_TO`, `AIMX_SUBJECT`, `AIMX_MAILBOX`, `AIMX_FILEPATH`, `AIMX_HOOK_NAME`. `after_send` env vars: `AIMX_FROM`, `AIMX_TO`, `AIMX_SUBJECT`, `AIMX_MAILBOX`, `AIMX_HOOK_NAME`, `AIMX_FILEPATH` (path to sent-copy `.md`), `AIMX_SEND_STATUS` (`delivered` | `failed` | `deferred`). Subprocess is spawned with `.env_clear()` + selective re-add of `PATH`, `HOME`, and the `AIMX_*` set (defense-in-depth).
- FR-31: Hooks fire on every event of their configured type. Recipient/subject matching is the author's responsibility to perform inside the shell `cmd` (e.g. with a `case` guard) — there are no match filters on the hook table itself.
- FR-32: Execute hooks synchronously — `on_receive` fires during delivery (after the mail is stored, per FR-16); `after_send` fires immediately after the outbound MX attempt resolves (success, failure, or deferred). The daemon awaits each subprocess to completion for predictable timing but discards the exit code — hooks cannot abort delivery or the send. Failures logged at `warn` with the effective hook name. Subprocess runtime > 5 s logged at `warn` for operator visibility into slow hooks.
- FR-32b: Every hook fire emits one `info`-level journald log line with the stable format `hook_name=<name> event=<e> mailbox=<m> (email_id=<id>|message_id=<id>) exit_code=<n> duration_ms=<n>`. This is the operator-level trace surface — there is no `hooks` field in email frontmatter.
- FR-32c: `aimx hooks create | list | delete` CLI manages hooks. `create` is flag-based (`--mailbox`, `--event`, `--cmd`, optional `--name`, optional `--dangerously-support-untrusted`). Omitting `--name` writes no `name =` line to config; the CLI prints the derived name on success. `list` prints a table (global view; `--mailbox <name>` filters) showing the effective name. `delete <name>` prompts interactively (`[y/N]`) showing the hook details unless `--yes`, and resolves against effective names (explicit or derived). No `update` verb — delete and recreate. `aimx hook` is retained as a clap alias.
- FR-32d: Hook CRUD routes through the daemon's UDS socket via `HOOK-CREATE` / `HOOK-DELETE` verbs on the `AIMX/1` codec; the daemon atomically rewrites `config.toml` (write-temp-then-rename) and hot-swaps the in-memory `Arc<Config>` so newly-created hooks fire on the very next event. `HOOK-DELETE` carries a `Hook-Name:` header. Clients fall back to direct `config.toml` edit + restart hint when the daemon is not running (same pattern as FR-18e MAILBOX-*).
...
12. **Hooks trust-gate inversion + `after_send`-only send-side hook** — Two events: `on_receive` (post-ingest) and `after_send` (new, post-delivery observability — exit code discarded, cannot abort the send). `before_send` was considered and dropped to protect outbound reliability — a flaky `before_send` hook could break mail delivery for the operator. The trust gate: `on_receive` hooks fire iff `trusted == "true"` on the email OR the hook sets `dangerously_support_untrusted = true`. A mailbox with `trust: none` fires no `on_receive` hooks by default — the per-hook opt-in is the escape hatch. Rationale: the v1 mailbox-level trust gate forced all-or-nothing per mailbox; the per-hook opt-in lets the same mailbox run trusted-only alerting plus a permissive log hook without conflict. Hooks carry an optional `name`; when omitted, AIMX derives a stable 12-char hex name from `event + cmd` so CRUD and log-grep (`hook_name=...`) work uniformly. Hook-fire traceability is provided via structured journald logs (`hook_name=... event=... exit_code=... duration_ms=...`) rather than a frontmatter `hooks` field — the frontmatter approach would have required either a second write per email or a pre-fire-only snapshot, neither of which reflected what actually ran.
