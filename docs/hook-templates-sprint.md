...
**Context:** PRD §6.6 exposes template-based creation via MCP, but operators also want to create template hooks from the CLI (e.g. for scripting). `aimx hooks create` grows two new flags: `--template NAME` (mutually exclusive with `--cmd`) and `--param KEY=VAL` (repeatable). When `--template` is set the CLI uses the UDS path — the same verb MCP uses — which stamps `origin = "mcp"` on every hook it writes. When `--cmd` is set the CLI takes the raw-cmd path (§S3-4) and stamps `origin = "operator"` via a direct `config.toml` write. When neither is set, the CLI prints a helpful error.

The template CLI path deliberately traverses UDS (not a direct-write) to keep the UDS verb the single source of truth for template validation (mailbox exists, template exists, params bound, event allowed, name unique). Duplicating that validation on the CLI side would mean two implementations of the same invariants. The consequence is that `origin` is a *submission channel* marker, not an authorship marker: anything written via UDS is `origin = mcp`, anything written via direct `config.toml` edit is `origin = operator`. The Sprint 5 tradeoff is that MCP `hook_delete` can remove CLI-template hooks; operators who want UDS-protected hooks can drop `--template` and use `--cmd`.
...
- [x] **(Sprint 2)** `spawn_via_systemd_run` preflight + fallback path emit two WARN lines on the same unknown-user + non-root event (one from the systemd preflight, one from the fallback delegation). Cosmetic, dev-box-only. Collapse to a single log line by either (a) silencing the systemd-path warn when it delegates, or (b) silencing the fallback warn when the systemd preflight already logged. *(from Sprint 2 re-review nit)* _Resolved in post-track cleanup: silenced the systemd preflight warn; fallback path's WARN is the single source of the signal._
- [x] **(Sprint 2)** `AIMX_SANDBOX_FORCE_FALLBACK` env var is test-only scaffolding but lives in production `src/platform.rs`. Consider gating it behind `#[cfg(any(test, debug_assertions))]` or at least documenting it more explicitly so an operator who stumbles on it doesn't think it's a supported switch. *(from Sprint 2 implementation note)* _Resolved in post-track cleanup: env lookup now gated behind `cfg(any(test, debug_assertions))`; release binaries ignore the variable entirely._
- [x] **(Sprint 2)** Unknown-user fallback for non-root callers (warns and runs as current UID) is a dev/CI safety net that production should never hit. Add a `doctor` check that flags this path if it ever fires in production logs. *(from Sprint 2 implementation note — track via `aimx doctor` work in Sprint 6)* _Resolved in post-track cleanup: `aimx doctor` now parses the 24h log tail for the fallback WARN and renders a `Sandbox:` warn line with a `sudo aimx setup` remediation hint when the count is non-zero._
- [x] **(Sprint 3)** `origin` field semantics: the daemon stamps `origin = "mcp"` on any UDS-submitted hook, including CLI `--template` invocations, making `origin` a submission-channel marker rather than an authorship marker. Sprint 5's `hook_delete` MCP tool description should note that it can remove CLI-template hooks; PRD §6.5 / §6.6 / §11 Q2 may want a one-paragraph clarification once Sprint 5 lands. *(from Sprint 3 review — AC line 176 intentionally relaxed to preserve "UDS is the single source of truth for template hooks")* _Resolved in post-track cleanup: `hook_delete` tool description spells out the submission-channel-vs-authorship distinction and names the CLI-template case explicitly; `book/mcp.md` carries the same paragraph. PRD left as-is to avoid post-launch churn._
- [x] **(Sprint 3)** `AIMX_TEST_SKIP_ROOT_CHECK` env var is a second test-only scaffolding switch living in production `src/hooks.rs` (mirrors the pattern of `AIMX_SANDBOX_FORCE_FALLBACK` from Sprint 2). Consolidate documentation / gating strategy for both when the Sprint 2 equivalent is addressed. *(from Sprint 3 implementation note)* _Resolved in post-track cleanup: both env vars now gated behind `cfg(any(test, debug_assertions))` via a shared `skip_root_check()` helper; release binaries ignore the variable entirely._
